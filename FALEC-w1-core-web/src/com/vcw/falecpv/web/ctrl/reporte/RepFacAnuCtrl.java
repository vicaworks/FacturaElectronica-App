...
public class RepFacAnuCtrl extends BaseCtrl {
	...
	public void initEvent(RepMainCtrl repMainCtrl) {
		try {
			...
			e.printStackTrace();
			AppJsfUtil.addErrorMessage("formMain2", "ERROR", TextoUtil.imprimirStackTrace(e, AppConfiguracion.getInteger("stacktrace.length")));
		}
	...
	private void consultar()throws DaoException {
		...
		ventasQueryList = consultaVentaServicio.getFacturasEmitidas(null,
	...
	public StreamedContent getFile() {
		try {
			...
			String path = FacesUtil.getServletContext().getRealPath(
					AppConfiguracion.getString("dir.base.reporte") + "FALECPV-FacAnuladas.xlsx");
			...
			rowCliente.createCell(6).setCellValue(establecimientoMain==null?"TODOS":TextoUtil.leftPadTexto(establecimientoMain.getCodigoestablecimiento(),3,"0") + " " + establecimientoMain.getNombrecomercial());
			...
			return AppJsfUtil.downloadFile(tempXls, "MAKOPV-FacAnuladas-" + establecimientoMain.getCodigoestablecimiento() + "_" + establecimientoMain.getNombrecomercial() + ".xlsx");
		} catch (Exception e) {
			e.printStackTrace();
			AppJsfUtil.addErrorMessage("formMain2", "ERROR", TextoUtil.imprimirStackTrace(e, AppConfiguracion.getInteger("stacktrace.length")));
		}
