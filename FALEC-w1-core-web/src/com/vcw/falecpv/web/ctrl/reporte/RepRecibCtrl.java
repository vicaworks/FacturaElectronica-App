...
public class RepRecibCtrl extends BaseCtrl {
	...
	public void buscar() {
		try {
			...
			ventasQueryList = consultaVentaServicio.getRecibos(AppJsfUtil.getEstablecimiento().getIdestablecimiento(), criterioBusqueda, desde, hasta);
			totalizar();
		} catch (Exception e) {
			e.printStackTrace();
			AppJsfUtil.addErrorMessage("formMain6", "ERROR", TextoUtil.imprimirStackTrace(e, AppConfiguracion.getInteger("stacktrace.length")));
		}
	}
